...
<form method="post" action="/supply_detail/update/{{$supply_detail->slogs_id}}/{{$supply_detail->book_id}}">
  <div class="form-group">
    ...
    <input type="text" class="form-control" name="book_id" placeholder="BOOK ID" value="{{$supply_detail->book_id}}" required>
  </div>
  <div class="form-group">
    <label class="col-form-label" for="formGroupExampleInput">AMOUNT</label>
    <input type="int" class="form-control" name="amount" placeholder="AMOUNT" value="{{$supply_detail->amount}}" required>
  </div>
